...
#define DEBUGMSG			0
#define DEBUGINBUF			0 // check input buffer data
...
#if (DEBUGMSG || DEBUGINBUF || DEBUGRX || DEBUGRXBUF || DEBUGTXBUF ||		\
		 DEBUGPUSH || DEBUGFLUS || DEBUGDSC || DEBUGBLK || DEBUGFETCH)
...
static void FTDI_WaitLongInRTI(int us)
{
	...
	if (nBytes) {
		...
		for (i = 0; i <= nBytes; i++)
			FTDI_AddByteToOutputBuffer(0, 0); // push N + 1 bytes
	}
	if (nBits)
		FTDI_WaitInRTI(nBits);
}
...
static DWORD FTDI_MoveJTAGStateTo(JtagStates JtagST2, DWORD Tailbit, int Is_Read)
{
	...
	if (DEBUGDSC) {
		DBG((" ---->Post ST mvoe.cur JTAG ST: %d -  TMS clk count: %d\n", CurJtagST, nTMSclks));
	}
	return nTMSclks;
}
...
{
	if (Is_clear)
		gFlushingBytelen = 0;         //clear Tx data buffer
...
static DWORD FTDI_AddDSCBlkToOutBuffer(int is_Instr, int optype, BYTE *buff, DWORD NumBitsToXfer, DWORD JtagST2)
{
	...
	if (optype == TAP_WO) {
		DSC_bytemode = DSC_BYTES_OUTN_LSB;
		DSC_bitmode = DSC_BITS_OUTN_LSB;
		DSC_tmsin = 0;
	} else if (optype == TAP_RO) {
		DSC_bytemode = DSC_BYTES_IN_LSB;
		DSC_bitmode = DSC_BITS_IN_LSB;
		DSC_tmsin = 1;
	} else if (optype == TAP_RW) {
		DSC_bytemode = DSC_BYTES_OUTN_IN_LSB;
		DSC_bitmode = DSC_BITS_OUTN_IN_LSB;
		DSC_tmsin = 1;
	} else
		return 0;
...
static int FTDI_scan_flush(int Is_clear)
{
	...
	if (Is_clear)
		gRxDataBufferIndex = 0;        //clean Rx data buffer
	...
	while (j < gInputBufferToRead) { // decode all Ro/RW data in input buffer to Rx data buffer
		...
		if (DEBUGRX) {
			for (g = 0; g < d; g++) {
				DBG(("decode Rx data [%d] = 0x%08X,0x%08X\n", gRxDataBufferIndex + g, *((DWORD *)(InputBuffer + j)), RxDataBuffer[gRxDataBufferIndex + g]));
			}
		}
...
static int FTDI_scan_xfer(void)
{
	...
	if (gTxDataBlkIndex == 0) //total blk to flush
		return 0;
	...
	if (DEBUGTXBUF) {
		...
		for (i = 0; i < gTxDataBlkIndex; i++) {
			DBG(("--> tx data buffer to flush, index [%8d],(T,L,V)= 0x%02X,%2d,0x%08X\n-> ",
			     i, TxDataBlkType[i], TxDataBlkBits[i], TxDataBuffer[i]));
			}
	}

	while (gTxFlushingBlkIndex < gTxDataBlkIndex) {
		if (DEBUGTXBUF) {
			DBG((" In Flushing at location %d, with total bytes %d\n", gTxFlushingBlkIndex, gTxDataBlkIndex));
		}
		if (gFlushingBytelen >= LL5) {
			if (DEBUGTXBUF) {
				DBG((" start Flushing ,%d bytes pending flush in output buffer\n", gFlushingBytelen));
			}
			st |= FTDI_scan_flush(0);
			if (DEBUGTXBUF) {
				DBG((" end Flushing ,at location %d, out buffer byte len %d\n", gTxFlushingBlkIndex, gFlushingBytelen));
			}
		} else
	...
	if (gFlushingBytelen > 0) {
		st |= FTDI_scan_flush(0);
		if (DEBUGTXBUF) {
			DBG((" final Flushing ,at location %d\n", gTxFlushingBlkIndex));
		}
	}

	if (DEBUGRXBUF) {
		for (i = 0; i < gRxDataBufferIndex; i++) {
...
static void loadftdll(void)
{
	...
	if (ftDll == NULL) {
		printf("Couldn't load ftd2xx.dll\n");
		exit(1);
	}
...
static void ft_getconfig(void)
{
	...
	if (LL4 == 0xFFFF)
		LL4 = FLASH_POLLING;
	if (!LL5)
		LL5 = MAX_NUM_BYTES_USB_WRITE_READ - 400;     //400 for final delay block reserve
	if (LL5 >= (MAX_NUM_BYTES_USB_WRITE_READ - 400))
		LL5 = MAX_NUM_BYTES_USB_WRITE_READ - 400;
	if (LL5 < 4096)
		LL5 = 4096;
	if (!LL6)
		LL6 = DMA_BLOCK_SIZE;
	if (!LL7)
		LL7 = 640;   //256->25418/22530 80%
	if (!LL8)
		LL8 = 80;    //32->31010/24386 80%
	if (LL9 == 1) { // safe mode
		LL1 = 59;
		LL2 = 10;
		LL4 = 128;
	...
	if (!GG1)
		GG1 = 0x14;   //GPIOLo, value 0x4, dir 0x1
	if (!GG2)
		GG2 = 0xFF;   //GPIOHi  value 0xF, dir 0xF
	//if (!USBID) USBID = 0x14575118;	//OpenMoko v2
	if (!USBID)
		USBID = 0x04038A98;	//TIAO TUMPA
	//printf("LL1[%d],LL2[%d],LL3[%d],LL4[%d],LL5[%d],LL6[%d],LL7[%d],LL8[%d],LL9[%d]\n",LL1,LL2,LL3,LL4,LL5,LL6,LL7,LL8,LL9);
}
...
int ftinit(void *p)
{
	...
	if ((devInfo != NULL) && (_FT_GetDeviceInfoList(devInfo, &dwNumDevs) == FT_OK)) {
		for (i = 0; i < dwNumDevs; i++) {
			...
			if (devInfo[i].ID == USBID && strstr(devInfo[i].Description, " A") != NULL) {
	...
	if (*opdir == XFER_ADD) { // ADD to queue op
		if (RWdir == XFER_TX) {
			...
			if (set_LL3) {
				...
				if (lowspeed)
					FTDI_AddBlkDelay(24);   // add 24us latency
			}
			...
			*opdir = XFER_QRY;
			return 0xffffffff;        //return add not success, all input buffer not empty
		}
		...
		if (RWdir == XFER_TX) {          //force flush the ouput queue, and flip to Rx opdir, fetch first
			if (BlockIndex <= 0) {
				*opdir = XFER_ADD;
				return 0xffffffff;   // no data flush, return
			}
		...
		do {
			...
			if (haveread && (dmadir == DMA_RD)) // data have read, second readable found. exit
				break;
			...
			if (set_LL3) {
...
#else
				{
					...
					gNextLocOnDataRead++;
					if ((cct & DSTRT) && (!silent_mode))
						printf("\nDMA didn't start... %08X, index %d\n", cct, BlockIndex);
					if (DEBUGFETCH) {
						if (cct & DSTRT) {
							DBG(("DMA didn't start at %d ...\n", i));
						}
			...
			if (set_LL3) {
...
#else
				{
					...
					cct = *(RxDataBuffer + gNextLocOnDataRead);
					gNextLocOnDataRead++;
					if ((cct & DERR) && (!silent_mode))
						printf("\nDMA error occurred... %08X, index %d\n", cct, BlockIndex);
					if (DEBUGFETCH) {
						if (cct & DERR) {
							DBG(("DMA error occurred %d ...\n", i));
						}
...
{
	int i, freebuffer, ilen, xfer_op = XFER_NOOP;
	DWORD data, addr;
...
{
	int i, j, freebuffer, ilen;
	int errflag = 1;
	int xfer_op = XFER_NOOP;
	DWORD addr, data, data_poll = 0;

	freebuffer = ft_dma_blkfetch(0, 0, 0, 0, &xfer_op);

	ilen = freebuffer / (9 + 2);  // can xfer so many dma block, guarentee buffer not overflow.

	if (len == 0) // answer back the caller query result for max supported block size
		return ilen;
